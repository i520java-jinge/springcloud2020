...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    public Payment getByIdForHystrixSuccess(Integer id) {
        log.info("----------->当前线程【"+Thread.currentThread().getName()+"】执行service的getByIdForHystrixSuccess方法。id："+id);
        return this.paymentDao.selectById(id);
    }

    @Override
    public Payment getByIdForHystrixTimeOut(Integer id) {

        try {
            Thread.sleep(5000); //沉睡3秒
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        log.info("----------->当前线程【"+Thread.currentThread().getName()+"】执行service的getByIdForHystrixTimeOut方法。耗时3秒！id："+id);
        return this.paymentDao.selectById(id);
    }
}